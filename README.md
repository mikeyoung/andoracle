...
Andoracle is an installable, offline-capable desktop duophonic browser synthesizer whose goal is a faithful functional recreation of the ARP Odyssey's normalized signal flow, playing behavior, and three filter characters—not component-for-component circuit simulation—with clearly separated modern additions including stereo delay, optional MIDI and live input, local named patches and note sequences, and URL sharing. Its audio engine is intentionally fixed at 44.1 kHz.

The interface is a fixed 1880 × 940 late-1960s hardware console shown in a strict orthographic overhead view. One generated full-console photograph supplies its continuous enamel chassis, physical seams, mounting hardware, keyboard well, wear, and lighting; generated transparent photographs supply its knobs, dial scales, phenolic buttons, selector marks, and piano keys. The on/off hardware uses the approved photographic bat-handle switch states from the sibling Chaotic Sound Effects project. Faceplate labels are raster-rendered while changing values remain live system text. The complete console scales as one unit to the browser content viewport, with no responsive reflow or page scrolling.
...
- Right-click or press and hold a persistent parameter to enter an exact value and see its valid range.
...
- Use **Panic** to release every physical note and PPC gesture, turn off the auto gate, and hard-clear the synth and stereo-delay state without cycling Power.
...
- Enable **Trails** to route the delay after the final VCA so repeats decay independently after keyboard release. With Trails off, repeats follow the VCA release and the delay starts the next keyboard phrase with an empty buffer.
...
With Trails off, the complete internal-source and optional-live-input mix enters the user-controlled stereo delay before matched left/right instances of the selected resonant low-pass character, manual high-pass, shared VCA control, drive, safety limiter, and master output. Key-up leaves the delay memory intact through the current VCA release; the ring retires when that VCA fully closes, and the next key-down is a final clean-start boundary. Trails moves the delay after the final VCA and drive so repeats can decay independently after release without exposing the synth's free-running oscillators. An optional output-feedback return models the output-to-external-input patch through the complete active route.
